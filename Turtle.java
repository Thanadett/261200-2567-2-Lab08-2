...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    {
        TurtleThread1 turtle1 = new TurtleThread1("green",0, 0);
        TurtleThread2 turtle2 = new TurtleThread2("pink",0, 0);
        TurtleThread3 turtle3 = new TurtleThread3("yellow",0, 0);
    ...
    static class TurtleThread1 extends Thread {
        private Turtle turtle;
        private String color;
        private double startX, startY;

        public TurtleThread1(String color, double startX, double startY) {
            this.color = color;
            this.startX = startX;
            this.startY = startY;
        }
...
//            turtle.hide();
    ...
    static class TurtleThread2 extends Thread {
        private Turtle turtle;
        private String color;
        private double startX, startY;

        public TurtleThread2(String color, double startX, double startY) {
            this.color = color;
            this.startX = startX;
            this.startY = startY;
        }
        ...
        public void run() {
            ...
            for (int i = 0; i < 12; i++) {
                ...
                for (int j = 0; j < 2; j++) {
                    for (int k = 0; k < 60; k++) {
                        turtle.forward(200 * Math.PI / 180);
                        turtle.right(1);
                    }
                    turtle.right(120);
                }
...
//            turtle.hide();
    ...
    static class TurtleThread3 extends Thread {
        private Turtle turtle;
        private String color;
        private double startX, startY;

        public TurtleThread3(String color, double startX, double startY) {
            this.color = color;
            this.startX = startX;
            this.startY = startY;
        }
...
//        System.out.println(Arrays.toString(fontNames));
    ...
    private static void setupBuffering()
    {
        synchronized(turtleLock)
        {
            ...
            midscreen.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
            midscreen.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BICUBIC);
            midscreen.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            midscreen.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL, RenderingHints.VALUE_STROKE_PURE);

            onscreen.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
            onscreen.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BICUBIC);
            onscreen.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            onscreen.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL, RenderingHints.VALUE_STROKE_PURE);
